...
应用切换不经过空白页。管理器、设置、商店和普通应用页面使用黑白质量刷新；
只有壁纸、彩色图片等明确的彩色内容才使用彩色 waveform。进入、暂停和召回应用
不会自动升级为彩色全屏刷新，完整刷新只保留给锁屏提交、手动清除残影等明确边界。

锁屏设置可选择白纸、原生锁屏或自定义 PNG，并配置裁切方式和自动休眠时间；
正式锁屏只显示壁纸，不显示时钟、提示或返回按钮，也不接受触摸输入。设置写入
...
开发不以“每修一个问题就完整发版”为循环。先在一台代表设备上完成定向快循环，
问题收敛后才进入双机、全量检查和正式 Release 的慢循环。USB 传输优先于设备 Wi-Fi，
测试按基础、故障、压力、功耗逐层放行；前一层失败时不重复运行后续层。

具体的变更分级、命令模板、热部署边界、停止条件和发布门禁见
[`docs/FAST_ITERATION.md`](docs/FAST_ITERATION.md)。任何修改发布流程或实机测试矩阵的
变更，都必须同步更新该文档，避免重新退回“改一处、全跑一次、再猜下一处”的流程。

### CI/CD 发布

普通分支与 Pull Request 由 `.github/workflows/ci.yml` 执行完整 host 检查。正式系统
发布只需更新工作区版本与 `release/sequence`，然后推送同版本 tag（例如
`v0.1.6`）。`.github/workflows/release.yml` 会自动完成：
